...
//----------------------------------------------------------------------------------------------------------------------
    ...
    internal void SetLight(LightData lightData, bool syncVisibility, bool shouldUpdate) {
        ...
        if (flags.hasColor) {
            // Blender sends light colors in linear RGB. Unity's Light.color is authored
            // as the color picker value, and URP/HDRP converts it to the active render
            // color space during lighting. Convert to gamma in Linear projects so the
            // final rendered light color matches the Blender linear value.
            Color syncedColor = QualitySettings.activeColorSpace == ColorSpace.Linear
                ? lightData.color.gamma
                : lightData.color;

            destLight.color = syncedColor;
#if AT_USE_HDRP
            destHDLightData.color = syncedColor;
#endif
        }
